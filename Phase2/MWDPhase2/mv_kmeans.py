...
for objectIndex in range(1,14,1):
    ...
    for qframeNo in np.nditer(qframeNos):
        qframe = query[query[:, 1] == qframeNo, 2:4];
        frameSim = np.array([]).reshape(0,3);
        for oframeNo in np.nditer(oframeNos):
            oframe = object[object[:, 1] == oframeNo, 2:4];
            frameD = cdist(qframe, oframe, 'jaccard');
...
print vdMat
np.savetxt('testkm.csv', vdMat, delimiter=',',fmt="%d, %.4f")
print 'stop here'